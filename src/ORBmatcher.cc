...
namespace ORB_SLAM3
{
    ...
    float ORBmatcher::TH_HIGH = 100;
    float ORBmatcher::TH_LOW = 50;
...
// http://graphics.stanford.edu/~seander/bithacks.html#CountBitsSetParallel
    float ORBmatcher::DescriptorDistance(const cv::Mat &a, const cv::Mat &b)
    {
        if(a.type() == CV_32F)
        {
            // L2 distance for float descriptors (SuperPoint, L2-normalized)
            // L2^2 = 2 - 2*dot(a,b), return L2
            float dot = a.dot(b);
        ...
        {
            // Original Hamming distance for ORB descriptors (CV_8U)
